...
describe('BookCategoryControllerAdmin (e2e)', () => {
  ...
  it('POST /admin/book-category -> should return 401', async () => {
    const res = await request(app.getHttpServer()).post('/admin/book-category').send({ title: 'Tarix' }).expect(401);
  });

  // it('POST /auth/sign-in -> should respond with a jwt token and return 201', async () => {
  //   const res = await request(app.getHttpServer())
  //     .post('/auth/sign-in')
  //     .send({ login: 'solihcoder@gmail.com', password: '12345' })
  //     .expect(201);
